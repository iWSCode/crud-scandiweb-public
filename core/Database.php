...
class Database
{
    ...
    {
        $this->pdo = new PDO('mysql:host=' . DB_HOST . ';dbname=' . DB_NAME . ';port=3306;', DB_USER, DB_PASSWORD);
        $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $statement = $this->pdo->query("SELECT COUNT(*) FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = 'u187663721_user';");
        if (!$statement->fetchColumn()) {
            $statement = $this->pdo->prepare(
                "CREATE DATABASE u187663721_user;
                USE u187663721_user;
        ...
        $this->pdo->query("USE u187663721_user;");
    }
